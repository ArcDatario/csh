...
<div class="orders-sidebar">
    ...
    <a href="processing-order" class="sidebar-item active">
    ...
    <a href="cancelled-orders" class="sidebar-item">
        ...
        <div class="quotes-container processing-orders-container" id="processing-orders-container" style="display:block;">
            ...
            if ($user_id) {
                ...
                // Count total processing orders
                $count_sql = "SELECT COUNT(*) AS total FROM orders WHERE user_id = ? AND status = 'processing'";
                ...
                $sql = "SELECT orders.*, users.name, users.phone_number 
                        ...
                        WHERE orders.user_id = ? AND orders.status = 'processing' 
...
function displayProcessingSearchResults(data, searchTerm, currentPage) {
    ...
    if (orders.length === 0) {
        const message = searchTerm 
            ? `No processing orders found for ticket "${searchTerm}"`
    ...
    orders.forEach(order => {
        ...
        const statusClass = order.status === 'processing' ? 'status-approved' : 'status-pending';
        const statusText = order.status === 'processing' ? 'Processing' : 'Pending';
    ...
    if (totalPages > 0) {
        html += '<div class="pagination" id="processing-search-pagination">';
        
        // Previous button
        if (currentPage > 1) {
            html += `<a href="javascript:void(0)" class="page-btn prev-next" onclick="searchProcessingOrders('${escapeHtml(searchTerm)}', ${currentPage - 1})">‹ Prev</a>`;
        } else {
        ...
        // First page
        if (currentPage > 3) {
            html += `<a href="javascript:void(0)" class="page-btn" onclick="searchProcessingOrders('${escapeHtml(searchTerm)}', 1)">1</a>`;
            if (currentPage > 4) {
        ...
        for (let i = startPage; i <= endPage; i++) {
            if (i === currentPage) {
                ...
                html += `<a href="javascript:void(0)" class="page-btn" onclick="searchProcessingOrders('${escapeHtml(searchTerm)}', ${i})">${i}</a>`;
            }
        }
        ...
        if (currentPage < totalPages - 2) {
            ...
            html += `<a href="javascript:void(0)" class="page-btn" onclick="searchProcessingOrders('${escapeHtml(searchTerm)}', ${totalPages})">${totalPages}</a>`;
        }
        
        // Next button
        if (currentPage < totalPages) {
            html += `<a href="javascript:void(0)" class="page-btn prev-next" onclick="searchProcessingOrders('${escapeHtml(searchTerm)}', ${currentPage + 1})">Next ›</a>`;
        } else {
    ...
    document.querySelectorAll('.view-details-btn.processing-order-btn').forEach(button => {
        button.addEventListener('click', openProcessingOrderModal);
    });
}

// Clear processing search functionality
function clearProcessingSearch() {
    const searchInput = document.getElementById('ProcessingSearchInput');
    const clearSearchBtn = document.getElementById('clearProcessingSearch');
    ...
    if (searchInput) {
        ...
        window.location.href = 'processing-order?page=1';
    }
}
