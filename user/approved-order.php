...
<div class="quotes-container approved-orders-container" id="approved-orders-container" style="display:block;">
    ...
    if ($user_id) {
        ...
        // Count total approved orders
        $count_sql = "SELECT COUNT(*) AS total FROM orders WHERE user_id = ? AND status = 'approved'";
            ...
            while ($order = $result->fetch_assoc()) {
                ...
                <div class="quote-card animate__animated animate__fadeInUp" data-ticket="<?= htmlspecialchars($order['ticket'], ENT_QUOTES, 'UTF-8') ?>">
                    ...
                    <div class="card-content">
                        ...
                        data-subtotal="<?= htmlspecialchars($order['pricing'] * $order['quantity'], ENT_QUOTES) ?>"
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (searchInput) {
        searchInput.addEventListener('input', function() {
            ...
            searchTimeout = setTimeout(() => {
                searchApprovedOrders(searchTerm, 1);
            }, 500); // 500ms delay
        });
    }
    ...
    if (clearSearchBtn) {
        clearSearchBtn.addEventListener('click', function() {
            ...
            searchApprovedOrders('', 1);
        });
    }
});

function searchApprovedOrders(searchTerm = '', page = 1) {
    const container = document.getElementById('approved-orders-container');
    
    if (!container) {
        console.error('Approved orders container not found');
        return;
    }
    ...
    console.log('Searching approved orders with URL:', searchUrl);
    ...
    fetch(searchUrl)
        ...
        .then(data => {
            console.log('Approved orders search response:', data);
            ...
            displayApprovedSearchResults(data, searchTerm, page);
        })
        .catch(error => {
            console.error('Approved orders search error:', error);
            container.innerHTML = '<div class="no-orders">Search failed: ' + error.message + '</div>';
        });
}

function displayApprovedSearchResults(data, searchTerm, currentPage) {
    const container = document.getElementById('approved-orders-container');
    ...
    if (totalPages > 0) {
        ...
        // Previous button
        if (currentPage > 1) {
            html += `<a href="javascript:void(0)" class="page-btn prev-next" onclick="searchApprovedOrders('${escapeHtml(searchTerm)}', ${currentPage - 1})">‹ Prev</a>`;
        } else {
        ...
        // First page
        if (currentPage > 3) {
            html += `<a href="javascript:void(0)" class="page-btn" onclick="searchApprovedOrders('${escapeHtml(searchTerm)}', 1)">1</a>`;
            if (currentPage > 4) {
        ...
        for (let i = startPage; i <= endPage; i++) {
            if (i === currentPage) {
                ...
                html += `<a href="javascript:void(0)" class="page-btn" onclick="searchApprovedOrders('${escapeHtml(searchTerm)}', ${i})">${i}</a>`;
            }
        }
        ...
        if (currentPage < totalPages - 2) {
            ...
            html += `<a href="javascript:void(0)" class="page-btn" onclick="searchApprovedOrders('${escapeHtml(searchTerm)}', ${totalPages})">${totalPages}</a>`;
        }
        
        // Next button
        if (currentPage < totalPages) {
            html += `<a href="javascript:void(0)" class="page-btn prev-next" onclick="searchApprovedOrders('${escapeHtml(searchTerm)}', ${currentPage + 1})">Next ›</a>`;
        } else {
...
function clearApprovedSearch() {
    ...
    if (searchInput) {
        ...
        window.location.href = 'approved-order?page=1';
    }
}
