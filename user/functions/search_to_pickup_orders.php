...
try {
    // Build query with search - Changed status to 'to-pick-up'
    $where_conditions = ["orders.user_id = ?", "orders.status = 'to-pick-up'"];
    $params = [$user_id];
    $param_types = "i";
    ...
    while ($order = $result->fetch_assoc()) {
        ...
        // Add pickup order specific fields
        if ($order['pickup_date']) {
            $order['pickup_date_formatted'] = date('M d, Y', strtotime($order['pickup_date']));
        } else {
            $order['pickup_date_formatted'] = '';
        }
        
        // Ensure all modal fields are present for pickup orders
        ...
        $order['pickup_attempt'] = $order['pickup_attempt'] ?? '';
        
        // Remove processing, shipping and completion dates since they're not relevant for pickup orders
