...
try {
    // Build query with search - Changed status to 'to_ship'
    $where_conditions = ["orders.user_id = ?", "orders.status = 'to_ship'"];
    $params = [$user_id];
    $param_types = "i";
    ...
    while ($order = $result->fetch_assoc()) {
        ...
        // Add to_ship order specific fields
        if ($order['shipping_date']) {
            $order['shipping_date_formatted'] = date('M d, Y', strtotime($order['shipping_date']));
        } else {
            $order['shipping_date_formatted'] = '';
        }
        
        // Ensure all modal fields are present for to_ship orders
        ...
        // Remove completion_date since it's not relevant for to_ship orders
