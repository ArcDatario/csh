...
try {
    // Build query with search - Changed status to 'processing'
    $where_conditions = ["orders.user_id = ?", "orders.status = 'processing'"];
    $params = [$user_id];
    $param_types = "i";
    ...
    while ($order = $result->fetch_assoc()) {
        ...
        // Add processing order specific fields
        if ($order['processing_date']) {
            $order['processing_date_formatted'] = date('M d, Y', strtotime($order['processing_date']));
        } else {
            $order['processing_date_formatted'] = '';
        }
        
        // Ensure all modal fields are present for processing orders
        ...
        // Remove shipping and completion dates since they're not relevant for processing orders
