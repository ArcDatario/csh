...
if ($report_type === 'excel') {
    ...
    $sheet->setCellValue("A{$rowNum}", "CSH ENTERPRISES");
    ...
    $sheet->mergeCells("A{$rowNum}:G{$rowNum}");
    $sheet->setCellValue("A{$rowNum}", date('F j, Y', strtotime($start_date)) . ' to ' . date('F j, Y', strtotime($end_date)));
    $sheet->getStyle("A{$rowNum}")->getFont()->setSize(10);
    $rowNum += 2;

    // Revenue summary
    if ($status === 'all') {
        $sheet->setCellValue("A{$rowNum}", "Total Revenue (Completed): ₱" . number_format($total_sales,2));
        $sheet->getStyle("A{$rowNum}")->getFont()->setBold(true);
        $rowNum++;
        $sheet->setCellValue("A{$rowNum}", "Estimated Revenue (Active): ₱" . number_format($estimated_sales,2));
        $sheet->getStyle("A{$rowNum}")->getFont()->setBold(true);
        $rowNum += 2;
    } elseif ($status === 'completed') {
        $sheet->setCellValue("A{$rowNum}", "Total Revenue (Completed): ₱" . number_format($total_sales,2));
    ...
    foreach ($groups_to_show as $group) {
        $sheet->setCellValue("A{$rowNum}", ucfirst($group) . " Orders");
        ...
        foreach ($orders[$group] as $o) {
            ...
            if ($group === 'active') $sheet->setCellValue("{$col}{$rowNum}", number_format($o['subtotal'] ?? 0,2)); $col++;
            if ($group === 'completed') $sheet->setCellValue("{$col}{$rowNum}", number_format($o['total'] ?? 0,2)); $col++;
