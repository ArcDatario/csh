...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_SESSION['admin_id'])) {
    ...
    try {
        ...
        // Update order status to mark as ready to ship and set processing to 'no'
        $query = "UPDATE orders SET status = 'to_ship', shipping_date = NOW() WHERE id = ?";
        ...
        try {
            ...
            // Content
            $mail->isHTML(true);
            $mail->Subject = 'Your Order #' . $ticket . ' is Ready to Ship';
            ...
            <body>
                ...
                <p>We are pleased to inform you that your order is now ready to be shipped. Below are the details:</p>
                ...
                <p>
                    Please prepare the amount of <strong>₱' . number_format($subtotal, 2) . '</strong>.
                </p>
            ...
            $mail->AltBody = "Dear Valued Customer,\n\n"
                . "We are pleased to inform you that your order is now ready to be shipped. Below are the details:\n\n"
                ...
                . "Please prepare the amount of ₱" . number_format($subtotal, 2) . ".\n\n"
                . "Best regards,\n"
                . "CSH Enterprises";
            ...
            echo json_encode(['success' => true, 'message' => 'Order marked as to ship and notifications sent successfully']);
        } catch (Exception $emailException) {
            // Rollback transaction if email fails but keep the database changes
            $conn->commit();
            error_log("Email sending failed: " . $emailException->getMessage());
            echo json_encode(['success' => true, 'message' => 'Order marked as to ship but email notification failed']);
        }
    } catch (Exception $e) {
        $conn->rollback();
